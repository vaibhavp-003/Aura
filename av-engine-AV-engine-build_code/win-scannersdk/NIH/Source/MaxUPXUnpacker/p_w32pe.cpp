...
extern CMaxPEFile g_objInputFile;
...
int PackW32Pe::canUnpack()
{
    ...
    isection = new pe_section_t[objs];
	fi->seek(pe_offset + fi->m_pInputFile.m_stPEHeader.SizeOfOptionalHeader + 0x18, SEEK_SET);
    fi->readx(isection,sizeof(pe_section_t)*objs);
    if (ih.objects < 3)
        return -1;
...
void PackW32Pe::rebuildImports(upx_byte *& extrainfo)
{
	...
	//zafeer31Jan code changed from +1 to +4
	if(get_le32(extrainfo)+4>obuf.getSize())
	{
		return;
	}
...
//    const upx_byte *p;
	...
	WORD wSecNo = 2;
	while(IDADDR(PEDIR_IMPORT) < isection[wSecNo].vaddr)
	{
		if(wSecNo == 0)
		{
			return;
	...
	if((IDADDR(PEDIR_IMPORT) - isection[wSecNo].vaddr) > ibuf.getSize())
	{
		return;
	}

    IPTR_I(const upx_byte, import, ibuf + IDADDR(PEDIR_IMPORT) - isection[wSecNo].vaddr);
	
	//IPTR_I(const upx_byte, import, ibuf + 0x41000-isection[0].vaddr);
    ...
    for (p = idata; get_le32(p) != 0; ++p)
    {
		...
		if(IDADDR(PEDIR_IMPORT) - isection[wSecNo].vaddr + get_le32(p)>ibuf.getSize())
		{
			return;
		}
         ICHECK(dname, 1);
		unsigned dlen=0;
		while(*(dname + dlen)!=0x00)
		{
			dlen++;
			if(dname+dlen > ibuf+ibuf.getSize())
			{
				return;
			}
		}
	...
	unsigned int atemp=ODADDR(PEDIR_IMPORT);
	if(unsigned int(ODADDR(PEDIR_IMPORT))+sizeof(import_desc) < rvamin || unsigned int(ODADDR(PEDIR_IMPORT))+sizeof(import_desc)>obuf.getSize())
	{
		return;
	}
...
//        LE32 *newiat = (LE32 *) (Obuf + iatoffs);
        ...
        for (p += 8; *p; ++newiat)
            if (*p == 1)
            {
                ...
                {
					if(*newiat + 2 + ilen + 1 - rvamin > obuf.getSize())
					{
						return;
					}
            ...
            else
            {
				...
				if(get_le32(p + 1) + import + 0x04 >ibuf + ibuf.getSize())
				{
					return;
				}
