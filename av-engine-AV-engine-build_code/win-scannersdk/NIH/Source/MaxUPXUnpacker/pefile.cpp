...
bool LZMA_Decompress(BYTE byLZMAProp[], DWORD dwSrcSize, DWORD dwDestSize, DWORD dwRead, DWORD dwWrite, CMaxPEFile *pMaxInFile, CMaxPEFile *pMaxOutFile, BYTE * byBuff = NULL);


////extern CMaxPEFile fi->m_pInputFile.;
//extern char g_szOutFilePath[MAX_PATH];
//extern bool fi->m_bCavityFlag;
//extern bool fi->m_bLZMA;

//bool g_bCheckForNewExe=false;
//DWORD dwStartRVA = 0x00;	
//bool g_bCheckForDiffVerDLL = true;

bool LZMA_Decompress(BYTE byLZMAProp[], DWORD dwSrcSize, DWORD dwDestSize, DWORD dwRead, DWORD dwWrite, CMaxPEFile *pMaxInFile, CMaxPEFile *pMaxOutFile, BYTE * byBuff);
...
int PeFile::readFileHeader()
{
    ...
    int ic;
    pe_offset = 0;
	unsigned int iEarlierpeOffset=0x00;
    for (ic = 0; ic < 20; ic++)
    {
        ...
        if (h.mz == 'M' + 'Z'*256) // dos exe
        {
            ...
            else
			{
				/*if(bflag)
				{
					iEarlierpeOffset = pe_offset;
					bflag=false;
				}*/
				LE32 dwOffset=h.nexepos;
				pe_offset += h.p512*512+h.m512 - h.m512 ? 512 : 0;
				fi->seek(pe_offset,SEEK_SET);
				fi->readx(&h,sizeof(h));
				if((get_le32(&h) == 'P' + 'E'*256))
				{
					break;
				}
				else
...
bool PeFile::Export::convert(unsigned eoffs,unsigned esize,unsigned boffset,unsigned bsize)
{
	char byTemp[sizeof(export_dir_t)] = {0};

	if(memcmp(base + eoffs, byTemp, sizeof(export_dir_t)) == 0)
	{
		return false;
	}
	...
	if(boffset && bsize)
	{
		if(unsigned((unsigned char*)base)+eoffs<boffset || unsigned((unsigned char*)base)+eoffs+sizeof(export_dir_t)>boffset+bsize)
		{
			memset(&edir, 0, sizeof(export_dir_t));
			return false;
		}    
	}
	...
	if(boffset && bsize)
	{
		if(unsigned((unsigned char*)base)+edir.name<boffset || unsigned((unsigned char*)base)+edir.name>boffset+bsize)
		{
			memset(&edir, 0, sizeof(export_dir_t));
			return false;
		}
...
void PeFile::processExports(Export *xport,unsigned int iBufbase,unsigned int ibufSize) 
{
	...
	if(!xport->convert(IDADDR(PEDIR_EXPORT),IDSIZE(PEDIR_EXPORT),iBufbase,ibufSize))
	{
        soexport = 0;
		return;
	}
...
PeFile::Resource::Resource(const upx_byte *p, int iResSecSize)
{
    init(p, iResSecSize);
...
void PeFile::Resource::init(const upx_byte *res, int iResSecSize)
{
    ...
    dsize = ssize = 0;
	bResourceInvalid = false;
	iResourceCounter = 0;
    if(check((const res_dir*) start,0, iResSecSize))
	{
		root = convert(start,NULL,0, iResSecSize);
	}
}

bool PeFile::Resource::check(const res_dir *node,unsigned level, int iResSecSize)
{
	int ic = node->identr + node->namedentr;
	iResourceCounter+=sizeof(res_dir)+(ic*0x08)-0x08;
	if (ic == 0)
		return true;
	for (const res_dir_entry *rde = node->entries; --ic >= 0; rde++)
	{
		...
		if(iResourceCounter!=(rde->child & 0x7fffffff))
		{
			bResourceInvalid = true;
			return false;
		}
		...
		if (level != 2)
		{
			if((rde->child & 0x7fffffff) > iResSecSize)
			{
				bResourceInvalid = true;
				return false;			
			}
			check((const res_dir*) (start + (rde->child & 0x7fffffff)),level + 1, iResSecSize);
		}
		else
		{
			iResourceCounter+=0x10;
		}
	}
	if(bResourceInvalid)
	{
													   ...
													   unsigned iRessize)
{
	...
	memset(branch, 0, sizeof(upx_rbranch));
    branch->name = NULL;
    ...
    {
		upx_rnode *child = convert(start + (rde->child & 0x7fffffff),branch,level + 1, iRessize);
		if(!child)
			break;
...
void PeFile::processResources(Resource *res)
{
    ...
    if (opt->win32_pe.compress_rt[_RT_STRING] < 0)
    {
        // by default, don't compress _RT_STRINGs of screensavers (".scr")
        opt->win32_pe.compress_rt[_RT_STRING] = true;
        if (fn_has_ext(fi->getName(),"scr"))
            opt->win32_pe.compress_rt[_RT_STRING] = false;
    }

	int iResSecSize = isection[ih.objects - 1].vsize;
    res->init(ibuf + vaddr, iResSecSize);
    ...
    if (opt->win32_pe.compress_icons == 2)
        while (res->next()) // there is no rewind() in Resource
            if (res->itype() == _RT_GROUP_ICON && iconsin1stdir == 0)
            {
    ...
    if (opt->win32_pe.compress_icons == 1)
        while (res->next())
            if (res->itype() == _RT_GROUP_ICON && first_icon_id == (unsigned) -1)
    ...
    while (res->next())
    {
        ...
        else if (rtype == _RT_GROUP_ICON)    // icon directory
        ...
        if (rtype == _RT_ICON && opt->win32_pe.compress_icons == 1)
            compress_icon = true;
        else if (rtype == _RT_GROUP_ICON)
        {
            if (opt->win32_pe.compress_icons == 1)
            {
...
//    upx_byte *p = rdata;
    ...
    if (opt->win32_pe.strip_relocs && !isdll)
    {
		if(obuf.getSize()<ODSIZE(PEDIR_RELOC))
		{
...
void PeFile::rebuildExports(unsigned wSecNo)
{
	...
	Export xport((char*)(unsigned char*) ibuf - isection[wSecNo].vaddr);
	processExports(&xport,unsigned((unsigned char*)ibuf),ibuf.getSize());
	if(soexport != 0)
	{
...
void PeFile::rebuildResources(upx_byte *& extrainfo, unsigned wSecNo)
{
	...
	const unsigned vaddr = IDADDR(PEDIR_RESOURCE);
	const upx_byte *r=NULL;
	if(vaddr<isection[wSecNo + 1].vaddr || vaddr-isection[wSecNo+1].vaddr>ibuf.getSize())
	{
		return;
	}

	r = ibuf - isection[wSecNo + 1].vaddr;
	
	int iResSecSize = isection[wSecNo + 1].vsize;
	Resource res(r + vaddr, iResSecSize);
	if(res.bResourceInvalid)
	{
		return;
	}
	while (res.next())
	{
		if (res.offs() > isection[wSecNo + 1].vaddr && (res.offs()- isection[wSecNo + 1].vaddr-4) < iResSecSize)
		{
			unsigned origoffs = get_le32(r + res.offs() - 4);
			res.newoffs() = origoffs;
				
			if(origoffs < rvamin || origoffs - rvamin + res.size() > obuf.getSize() || res.offs() - isection[wSecNo + 1].vaddr + res.size() > ibuf.getSize())
			{
				return;
			}
			omemcpy(obuf + origoffs - rvamin,r + res.offs(),res.size());
			if (icondir_count && res.itype() == _RT_GROUP_ICON)
			{
	...
	if(ODADDR(PEDIR_RESOURCE) - rvamin + 12 + 4 > obuf.getSize())
	{
		return;
	}
	...
	{
		if(ODADDR(PEDIR_RESOURCE) - rvamin + res.dirsize() > obuf.getSize())
		{
			return;
		}
...
void PeFile::unpack(OutputFile *fo)
{
	...
	unsigned overlay = file_size - ALIGN_UP(isection[iobjs - 1].rawdataptr + isection[iobjs - 1].size, ih.filealign);
	if(file_size == isection[iobjs - 1].rawdataptr + isection[iobjs - 1].size || overlay == file_size || 
		file_size < ALIGN_UP(isection[iobjs-1].rawdataptr+isection[iobjs - 1].size, ih.filealign))
	{
		overlay=0;
	}

	//**************Change made here upx**********///Removing this check
	//checkOverlay(overlay);

	DWORD dwOffset = 2;
	DWORD dwStartRVA=0;
		
	if(!fi->m_pInputFile.ReadBuffer(&dwStartRVA , fi->m_pInputFile.m_dwAEPMapped, sizeof(DWORD), sizeof(DWORD)))
	{
		return;
	}
	
	if(*(BYTE*)&dwStartRVA == 0xBE && (fi->m_pInputFile.m_stPEHeader.Characteristics & IMAGE_FILE_DLL) != IMAGE_FILE_DLL)
	{
		dwOffset-=0x01;
	}
	else if(*(WORD*)&dwStartRVA != 0xBE60 && (fi->m_pInputFile.m_stPEHeader.Characteristics & IMAGE_FILE_DLL) == IMAGE_FILE_DLL)
	{
		dwOffset+=0x0B;
	}	
	else if((fi->m_pInputFile.m_stPEHeader.Characteristics & IMAGE_FILE_DLL) == IMAGE_FILE_DLL)
	{
		fi->m_bCheckForDiffVerDLL = false;
	}

	if(!fi->m_pInputFile.ReadBuffer(&dwStartRVA , fi->m_pInputFile.m_dwAEPMapped + dwOffset, sizeof(DWORD), sizeof(DWORD)))
	{
		return;
	}
	unsigned wSecNo = fi->m_pInputFile.Rva2FileOffset(dwStartRVA  - fi->m_pInputFile.m_stPEHeader.ImageBase, &dwStartRVA);
	if(wSecNo == 0xFF || wSecNo > 0x02)
	{
		return;
	}
	if (fi->m_bCavityFlag || ph.c_len==0x00)
	{
		ph.c_len = fi->m_pInputFile.m_stSectionHeader[wSecNo].Misc.VirtualSize;
		if(ph.c_len>fi->m_pInputFile.m_dwFileSize-fi->m_pInputFile.m_stSectionHeader[wSecNo].PointerToRawData)
		{
			ph.c_len = fi->m_pInputFile.m_dwFileSize - fi->m_pInputFile.m_stSectionHeader[wSecNo].PointerToRawData;
		}
	}
	if(ph.c_len==0x00)
	{
		ph.c_len = fi->m_pInputFile.m_stSectionHeader[wSecNo].SizeOfRawData;
		ph.u_len += fi->m_pInputFile.m_stSectionHeader[wSecNo].SizeOfRawData;
	}
	if(ph.c_len==0x00 || ph.u_len==0x00)
	{
		return;
	}
	ibuf.alloc(ph.c_len);
	...
	fi->seek(dwStartRVA, SEEK_SET);
	int iReadBytes=fi->readx(ibuf,ph.c_len);
	if(iReadBytes==0xFFFFFFFF)
	{
		return;
	}
	...
	if(M_IS_LZMA(ph.method))
	{
		BYTE byLZMAProp[4]={0};
		DWORD dwOffset=0x37;
		if((fi->m_pInputFile.m_stPEHeader.Characteristics & IMAGE_FILE_DLL)==IMAGE_FILE_DLL && fi->m_bCheckForDiffVerDLL)
		{
			dwOffset += 0x0B;
		}

		if((fi->m_pInputFile.m_stPEHeader.Characteristics&IMAGE_FILE_DLL) != IMAGE_FILE_DLL && fi->m_pInputFile.ReadBuffer(byLZMAProp, fi->m_pInputFile.m_dwAEPMapped + 0x0C, 0x04, 0x04))
		{
			if(*(WORD*)&byLZMAProp[0]==0x87C7)
			{
				dwOffset+=0x0A;
			}
		}

		if(fi->m_pInputFile.ReadBuffer(byLZMAProp,fi->m_pInputFile.m_dwAEPMapped + dwOffset, 0x04, 0x04))
		{		
			if(LZMA_Decompress(byLZMAProp, ph.c_len, ph.u_len, dwStartRVA + 0x02, 0, &fi->m_pInputFile, NULL, obuf))
			{
				//r = UPX_E_OK;
			}
			else
	...
	else
	{
		...
		int r = upx_decompress(ibuf, ph.c_len, obuf, &new_len, ph.method, &ph.compress_result);
		if (r == UPX_E_OUT_OF_MEMORY || (r != UPX_E_OK && r != UPX_E_INPUT_NOT_CONSUMED && r != UPX_E_LOOKBEHIND_OVERRUN))
		{
			return;
		}
		ph.u_len = new_len;
	...
	upx_byte *extrainfo = obuf;
	unsigned LE32Address = get_le32(obuf + ph.u_len - 4);
	if((get_le32(obuf + ph.u_len - 4)&0xFF000000)==0xcd000000)
	{
		LE32Address = 0;
		bool bflag=false;
		upx_byte *temp=obuf + ph.u_len-4;
		unsigned counter=0;		
		unsigned totalcount=0x200;
		if((ph.u_len&0xFFFF)<0x200)
		{
			totalcount=ph.u_len;
		}		
		while(counter<totalcount-5)
		{
			if(*(temp-counter)==0x45 && *(temp-counter-1)==0x50)
			{
				bflag=true;				
				break;
			}
	...
	{
		extrainfo+= get_le32(obuf + ph.u_len - 4); 
	}

	if(LE32Address > ph.u_len || (*(unsigned long *)extrainfo) != 0x4550)
	{
		TCHAR szOutFilePath[256] = {0};
		char szOutFile[256] = {0};
		strcpy_s(szOutFile, 256, fo->getName());
		_stprintf_s(szOutFilePath, 256, L"%S",fo->getName());
		fo->closex();

		CopyFile(fi->m_pInputFile.m_szFilePath,szOutFilePath,false);
		
		fo->sopen(szOutFile,O_WRONLY,SH_DENYNO,0600);
		fo->seek(fi->m_pInputFile.m_stSectionHeader[0].PointerToRawData,SEEK_SET);
		fo->write(obuf,ph.u_len);
		fo->seek(fi->m_pInputFile.m_stPEHeader.e_lfanew+sizeof(DWORD)+sizeof(IMAGE_FILE_HEADER)+0x10,SEEK_SET);
		unsigned dwAEP=0x101A;
		fo->write(&dwAEP,sizeof(DWORD));
		fo->seek(fi->m_pInputFile.m_stPEHeader.e_lfanew+sizeof(DWORD)+sizeof(IMAGE_FILE_HEADER)+fi->m_pInputFile.m_stPEHeader.SizeOfOptionalHeader+0x10,SEEK_SET);
		fo->write(&ph.u_len,sizeof(DWORD));	
		return;
	}
	...
	if(extrainfo + (sizeof(pe_section_t) * objs) >  obuf + obuf.getSize())
	{
		return;
	}

	memcpy(osection,extrainfo,sizeof(pe_section_t) * objs);
	rvamin = osection[0].vaddr > 0x1000 ? 0x1000 : osection[0].vaddr;
	...
	while(IDADDR(PEDIR_IMPORT) < isection[wSecNo+1].vaddr)
	{
		if(wSecNo == 0xFFFFFFFF)
		{
			break;
		}
		wSecNo -= 1;
	...
	// unfilter
		if (ph.filter)
		{
			...
			//zafeer31Jan below code removed
			//if(oh.codebase == 0x00)
			//{
			//	return;
			//}
			...
			if(oh.codebase != 0x00)
			{
				if(oh.codesize < oh.codebase - rvamin + obuf.getSize() && oh.codebase-rvamin+oh.codesize <= obuf.getSize())
				{
	...
	if(!fi->m_bLZMA && isection[wSecNo + 1].size!=0x00 && wSecNo != 0xFFFFFFFF && (fi->m_pInputFile.m_dwFileSize!=isection[wSecNo + 1].rawdataptr) )
	{		
		if(isection[wSecNo + 1].size > 1024*1024*10)
		{
			return;
		}

		if(fi->m_pInputFile.m_dwFileSize-isection[wSecNo + 1].rawdataptr<isection[wSecNo + 1].size)
		{
			isection[wSecNo + 1].size=fi->m_pInputFile.m_dwFileSize-isection[wSecNo + 1].rawdataptr;
		}

		ibuf.alloc(isection[wSecNo + 1].size);		

		fi->seek(isection[wSecNo + 1].rawdataptr,SEEK_SET);
		iReadBytes=fi->readx(ibuf,isection[wSecNo + 1].size);
		if(iReadBytes==0xFFFFFFFF)
		{
			return;
		}
		...
		rebuildTls();
		rebuildExports(wSecNo + 1);

		// Change made here--upx********/
		ih.objects=3;
		//rebuildResources(extrainfo, wSecNo);
		DWORD dwFileOffset=IDADDR(PEDIR_RESOURCE);
		WORD wResourcesSecNo= fi->m_pInputFile.Rva2FileOffset(dwFileOffset,&dwFileOffset);
		if (ODSIZE(PEDIR_RESOURCE) != 0 || IDSIZE(PEDIR_RESOURCE) != 0)
		{
			if(wResourcesSecNo <= 0x03)
			{
				if(wResourcesSecNo==0x00)
				{
					wResourcesSecNo=0x02;
				}
				if(wResourcesSecNo!=wSecNo+1)
				{
					ih.objects=3;
					ibuf.dealloc();

					if(fi->m_pInputFile.m_dwFileSize-isection[wResourcesSecNo].rawdataptr<isection[wResourcesSecNo].size)
					{
						isection[wResourcesSecNo].size=fi->m_pInputFile.m_dwFileSize-isection[wResourcesSecNo].rawdataptr;
					}

					ibuf.alloc(isection[wResourcesSecNo].size);
					fi->seek(isection[wResourcesSecNo].rawdataptr,SEEK_SET);
					int iReadBytes=fi->readx(ibuf,isection[wResourcesSecNo].size);
					if(iReadBytes==0xFFFFFFFF)
					{
						return;
					}
					wSecNo=wResourcesSecNo-1;
				}
				rebuildResources(extrainfo, wSecNo);
			}
		}
	...
	if (fo)
	{
		...
		ibuf.dealloc();
		//zafeer31Jan below code added as a check
		if(osection[ic].rawdataptr>1024 * 1024 * 20 || osection[ic].rawdataptr==0)
		{
			return;
		}
		...
		if(osection[ic].rawdataptr>fo->getBytesWritten())
		{
		...
		for (ic = 0; ic < objs; ic++)
		{
			unsigned int atemp=osection[ic].vaddr-rvamin+osection[ic].size;
			if (osection[ic].rawdataptr && unsigned(osection[ic].vaddr-rvamin+ALIGN_UP(osection[ic].size,oh.filealign))<obuf.getSize() && !(osection[ic].size&0x80000000))
			{
	...
	if(fi->m_bCheckForNewExe || pe_offset < 0x3C)
	{
		fo->seek(0x3C,SEEK_SET);
		fo->write(&fi->m_pInputFile.m_stPEHeader.e_lfanew,sizeof(LE32));
	}
	ibuf.dealloc();
}
	...
	while(1)
	{
		...
		if (inPos == inSize)
		{
			...
			else if(Counter == Loop_Length - 1)
			{
				...
				bySrcbuff = NULL;
				if(!(bySrcbuff = (BYTE *)malloc(inSize)))
				{
					break;
				}
			...
			if(!pMaxInFile->ReadBuffer(bySrcbuff, dwRead + (IN_BUF_SIZE * Counter), inSize, inSize))
			{				
				break;
			}
			Counter++;
		...
		if(thereIsSize && outProcessed > unpackSize)
		{
			...
			if(!(byDestbuff = (BYTE *)malloc(unpackSize % OUT_BUF_SIZE)))
			{
				break;
			}	
		...
		{
			NoRepeat = 0;
			if(!(byDestbuff = (BYTE *)malloc(outProcessed)))
			{
				break;
			}	
			memset(byDestbuff, 0x00, OUT_BUF_SIZE);
		}
		res = LzmaDec_DecodeToBuf(&state, byDestbuff + outPos, &outProcessed,bySrcbuff + inPos, &inProcessed, finishMode, &status, 0);

		inPos += inProcessed;
		outPos += outProcessed;
		unpackSize -= outProcessed;

		if((dwOffsetToWrite + outPos) > dwDestSize)
		{
			break;
		}
		if(byBuff)
		...
		{
			if(!pMaxOutFile->WriteBuffer(byDestbuff, dwWrite+dwOffsetToWrite, outPos, outPos))
			{
				break;
			}
		}
		...
		{
			bRet = (thereIsSize || status != LZMA_STATUS_FINISHED_WITH_MARK) ? true : false;
			break;
		}
	}
