...
static options_t _global_options;

/*************************************************************************
// process one file
**************************************************************************/

void do_one_file(const char *iname, char *oname,DWORD dwCodeOffset,DWORD dwSigOffset,bool bLZMA)
{
	opt = &_global_options;
	memset(opt, 0, sizeof(_global_options));
    ...
    if (opt->cmd == CMD_COMPRESS || opt->cmd == CMD_DECOMPRESS)
    {
	 ...
	 fo.sopen(oname,O_CREAT | O_WRONLY | O_BINARY,SH_DENYWR,0600);
...
void do_files(int i, int argc, char *argv[],unsigned long dwCodeOffset,unsigned long dwSigOffset,bool bLZMA)
{
    if (opt->verbose >= 1)
    {
        show_head();
    ...
    for ( ; i < argc; i++)
    {
        ...
        try {
            do_one_file(iname,oname,dwCodeOffset,dwSigOffset,bLZMA);
