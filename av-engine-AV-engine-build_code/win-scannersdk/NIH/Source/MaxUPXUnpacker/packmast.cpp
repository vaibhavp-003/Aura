...
//CMaxPEFile fi->m_pInputFile.;
//char g_szOutFilePath[MAX_PATH] = {0};
//bool fi->m_bCavityFlag = false;
...
void PackMaster::unpack(OutputFile *fo)
{
	...
	_stprintf_s(szFilePath, 256, L"%S", fi->getName());
	fi->m_pInputFile.OpenFile(szFilePath, false);
	
	//strcpy(g_szOutFilePath, fo->getName());
