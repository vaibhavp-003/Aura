...
//extern DWORD dwStartRVA;
//extern bool g_bCheckForDiffVerDLL;
...
void ph_decompress(PackHeader &ph, const upx_bytep in, upx_bytep out, bool verify_checksum, Filter *ft)
{
	...
	if (r != UPX_E_OK && r != UPX_E_INPUT_NOT_CONSUMED && r != UPX_E_LOOKBEHIND_OVERRUN)
	{
...
{
    if (!ph.fillPackHeader((unsigned char *)b, blen,fi))
        return false;
