...
protected:
    ...
    void processExports(Export *xport,unsigned int iBufbase, unsigned int ibufSize);
    void processExports(Export *,unsigned);
    void rebuildExports(unsigned);
    upx_byte *oexport;
    unsigned soexport;

    void processResources(Resource *);
    void processResources(Resource *, unsigned);
    void rebuildResources(upx_byte *&, unsigned wSecNo);
    upx_byte *oresources;
    ...
    class Resource : private noncopyable
    {
        ...
        bool check(const res_dir*,unsigned, int iResSecSize);
        upx_rnode *convert(const void *,upx_rnode *,unsigned, unsigned);
    ...
    public:
        Resource();
        Resource(const upx_byte *p, int iResSecSize);
        ~Resource();
        void init(const upx_byte *, int iResSecSize);
         ...
         */
		bool bResourceInvalid;
		int iResourceCounter;
    };
    ...
    public:
        ...
        bool convert(unsigned eoffs,unsigned esize,unsigned boffset,unsigned bsize);
