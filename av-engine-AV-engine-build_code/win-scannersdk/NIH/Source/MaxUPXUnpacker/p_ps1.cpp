...
bool PackPs1::getBkupHeader(unsigned char *p, unsigned char *dst)
{
    ...
    {
		unsigned int uiCompressedBuff = MemBuffer::getSizeForUncompression(SZ_IH_BKUP);	

		if ((int)uiCompressedBuff <= 0)
		{
			return false;
		}

        unsigned char *unc_bh = new unsigned char[uiCompressedBuff];
