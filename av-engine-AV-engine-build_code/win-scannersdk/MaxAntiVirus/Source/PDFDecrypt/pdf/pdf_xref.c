...
pdf_read_old_xref(fz_obj **trailerp, pdf_xref *xref, char *buf, int cap)
{
	...
	if(iCorruptNumber)
	{
		return fz_throw("xref table has non numeric characters");
	}
...
pdf_cache_object(pdf_xref *xref, int num, int gen)
{
	...
	else if (x->type == 'n')
	{
		...
		error = pdf_parse_ind_obj(&x->obj, xref, xref->file, xref->scratch, sizeof xref->scratch, &rnum, &rgen, &x->stm_ofs);
		if (error)
...
pdf_load_object(fz_obj **objp, pdf_xref *xref, int num, int gen)
{
	...
	*objp = fz_keep_obj(xref->table[num].obj);
	//*objp = fz_keep_obj(xref->table[g_xref_entry_num[num].entry_num].obj);
...
fz_error
pdf_open_xref(pdf_xref **xrefp, const wchar_t *filename, char *password)
{
	fz_error error;
	fz_stream *file;

	file = fz_open_file_w(filename);
	if (!file)
