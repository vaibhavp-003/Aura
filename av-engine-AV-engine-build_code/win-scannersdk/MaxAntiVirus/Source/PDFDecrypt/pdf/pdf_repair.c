...
pdf_repair_xref(pdf_xref *xref, char *buf, int bufsize)
{
	...
	while (1)
	{
		...
		if (tok == PDF_TOK_OBJ)
		{
			...
			else
			{
				...
				g_xref_entry_num[listlen].num = num;
				g_xref_entry_num[listlen].entry_num = listlen;
	...
	pdf_resize_xref(xref, listlen + 1);

	for (i = 0; i < listlen; i++)
	{
		xref->table[i].type = 'n';
		xref->table[i].ofs = list[i].ofs;
		xref->table[i].gen = list[i].gen;
		xref->table[i].stm_ofs = list[i].stm_ofs;
		/*
		...
		*/
		...
		if (list[i].stm_len >= 0)
		{
			...
			//error = pdf_load_object(&dict, xref, g_xref_entry_num[i].entry_num, list[g_xref_entry_num[i].entry_num].gen);
			error = pdf_load_object(&dict, xref, i, list[i].gen);
			if (error)
			{
				/*
...
pdf_repair_obj_stms(pdf_xref *xref)
{
	...
	for (i = 0; i < xref->len; i++)
	{
		if (xref->table[i].stm_ofs)
		{
			...
			if(!pdf_load_object(&dict, xref, g_xref_entry_num[i].num, 0))
			//if(!pdf_load_object(&dict, xref, i, 0))
			{
