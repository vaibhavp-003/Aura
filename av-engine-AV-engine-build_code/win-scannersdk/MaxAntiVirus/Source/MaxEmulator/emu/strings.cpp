...
int op_stos(Thread& thread,ins_disasm* s){
    ...
    for (int i=0;i< rep;i++){
        ...
        if (s->opcode->substr(0,s->opcode->size())=="stosb"){
           EMU_WRITE_MEM(thread.Exx[7],1,(char*)&thread.Exx[0]);
           thread.Exx[7]+=1;                                                
        }else if (s->opcode->substr(0,s->opcode->size())=="stosd" && (s->hde.flags & F_PREFIX_66)){
           EMU_WRITE_MEM(thread.Exx[7],2,(char*)&thread.Exx[0]);
           thread.Exx[7]+=2;
        }else if (s->opcode->substr(0,s->opcode->size())=="stosd" && !(s->hde.flags & F_PREFIX_66)){
           EMU_WRITE_MEM(thread.Exx[7],4,(char*)&thread.Exx[0]);
           thread.Exx[7]+=4;                                           
        };
    };
    return 0;
...
int op_movs(Thread& thread,ins_disasm* s){
    ...
    for (int i=0;i< rep;i++){
        ...
        if (s->opcode->substr(0,s->opcode->size())=="movsb"){
           ...
           EMU_WRITE_MEM(thread.Exx[7],1,(char*)&n);
           thread.Exx[6]+=1;
           thread.Exx[7]+=1;
        }else if (s->opcode->substr(0,s->opcode->size())=="movsd" && (s->hde.flags & F_PREFIX_66)){
           short n=(short)(*ptr &0xFFFF);
           EMU_WRITE_MEM(thread.Exx[7],2,(char*)&n);
           thread.Exx[6]+=2;
           thread.Exx[7]+=2;
        }else if (s->opcode->substr(0,s->opcode->size())=="movsd" && !(s->hde.flags & F_PREFIX_66)){
           dword n=*ptr;
           EMU_WRITE_MEM(thread.Exx[7],4,(char*)&n);
           thread.Exx[6]+=4;
           thread.Exx[7]+=4;                                           
        };
