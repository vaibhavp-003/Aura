...
BYTE HEADER_S2OS2O[24]		= {"MAXDBVERSION00.00.00.08"};
BYTE HEADER_S2OS2O_DATA[24]	= {0};
...
Author         : Anand Srivastava
--------------------------------------------------------------------------------------*/
CS2OS2O::CS2OS2O(bool bIsEmbedded): CBalBST(bIsEmbedded)
{
	m_bLoadError = false;
	m_bSaveError = false;
}
...
Author         : Anand Srivastava
--------------------------------------------------------------------------------------*/
COMPARE_RESULT CS2OS2O::Compare(ULONG64 dwKey1, ULONG64 dwKey2)
{
...
Author         : Anand Srivastava
--------------------------------------------------------------------------------------*/
void CS2OS2O::FreeKey(ULONG64 dwKey)
{
...
Author         : Anand Srivastava
--------------------------------------------------------------------------------------*/
void CS2OS2O::FreeData(ULONG64 dwData)
{
...
In Parameters  : LPCTSTR szKey, CS2OS2U * pObjS2OS2U, 
Out Parameters : bool 
Description    : add node in tree in ascending order in right vine
Author         : Anand Srivastava
--------------------------------------------------------------------------------------*/
bool CS2OS2O::AppendItemAscOrder(LPCTSTR szKey, CS2OS2U * pObjS2OS2U)
{
	...
	if(!AddNodeAscOrder((ULONG64)szHoldKey,(ULONG64)pObjS2OS2U->GetDataPtr()))
	{
...
Author         : Anand Srivastava
--------------------------------------------------------------------------------------*/
bool CS2OS2O::AppendItem(LPCTSTR szKey, CS2OS2U * pObjS2OS2U)
{
	LPTSTR szHoldKey = NULL;
	...
	if(!AddNode((ULONG64)szHoldKey,(ULONG64)pObjS2OS2U->GetDataPtr()))
	{
		return (false);
	}
...
Author         : Anand Srivastava
--------------------------------------------------------------------------------------*/
bool CS2OS2O::DeleteItem(LPCTSTR szKey)
{
...
/*--------------------------------------------------------------------------------------
Function       : SearchItem
In Parameters  : LPCTSTR szKey, CS2OS2U& objS2OS2U, 
Out Parameters : bool 
Description    : search a key in tree and return data
Author         : Anand Srivastava
--------------------------------------------------------------------------------------*/
bool CS2OS2O::SearchItem(LPCTSTR szKey, CS2OS2U& objS2OS2U)
{
	ULONG64 dwData = 0;
	...
	objS2OS2U.SetDataPtr((NODE*)dwData, m_pBuffer, m_nBufferSize);
	return (true);
}
...
Author         : Anand Srivastava
--------------------------------------------------------------------------------------*/
bool CS2OS2O::UpdateItem(LPCTSTR szKey, CS2OS2U& objS2OS2U)
{
...
Author         : Anand Srivastava
--------------------------------------------------------------------------------------*/
bool CS2OS2O::GetKey(PVOID pVPtr, LPCTSTR& szKey)
{
...
Author         : Anand Srivastava
--------------------------------------------------------------------------------------*/
bool CS2OS2O::GetData(PVOID pVPtr, CS2OS2U& objS2OS2U)
{
...
Author         : Anand Srivastava
--------------------------------------------------------------------------------------*/
DWORD CS2OS2O::ReadS2U(ULONG64*& pCurrentPtr, ULONG64 dwBaseAddress, DWORD dwFileSize)
{
...
Author         : Anand Srivastava
--------------------------------------------------------------------------------------*/
DWORD CS2OS2O::ReadS2O(ULONG64*& pCurrentPtr, ULONG64 dwBaseAddress, DWORD dwFileSize)
{
	DWORD dwBytesProcessed = 0;
	DWORD dwDataSize = 0;
	TCHAR * pStr = NULL;
...
Author         : Anand Srivastava
--------------------------------------------------------------------------------------*/
bool CS2OS2O::Load(LPCTSTR szFileName, bool bCheckVersion)
{
	DWORD dwDataSize = 0;
	TCHAR * pStr = NULL;
	...
	DWORD dwBytesRead = 0;
	BYTE VERSION_FROM_FILE[sizeof(HEADER_S2OS2O)] ={0};
	TCHAR szFullFileName[MAX_PATH]={0};
	BYTE byHeaderDataFromFile[sizeof(HEADER_S2OS2O_DATA)] ={0};
	BYTE byHeaderDataCalculated[sizeof(HEADER_S2OS2O_DATA)] ={0};
	...
	if(bCheckVersion && memcmp(HEADER_S2OS2O, VERSION_FROM_FILE, sizeof(VERSION_FROM_FILE)))
	{
		goto ERROR_EXIT;
	}
	...
	if(dwFileSize <= sizeof(HEADER_S2OS2O) + sizeof(HEADER_S2OS2O_DATA))
	{
		goto ERROR_EXIT;
	}

	dwFileSize -= sizeof(HEADER_S2OS2O) + sizeof(HEADER_S2OS2O_DATA);
	...
	dwBaseAddress -= sizeof(VERSION_FROM_FILE) + sizeof(HEADER_S2OS2O_DATA);
	...
	while(dwBytesProcessed < dwDataSize)
	{
		...
		dwBytesProcessed += sizeof(TCHAR);
		pCurrentPtr =(ULONG64*)pStr;
		dwBytesProcessed += ReadS2O(pCurrentPtr, dwBaseAddress, dwFileSize);
		if(m_bLoadError)goto ERROR_EXIT;
	}
...
Author         : Anand Srivastava
--------------------------------------------------------------------------------------*/
DWORD CS2OS2O::DumpS2U(HANDLE hFile, ULONG64 dwData)
{
...
Author         : Anand Srivastava
--------------------------------------------------------------------------------------*/
DWORD CS2OS2O::DumpS2O(HANDLE hFile, ULONG64 dwData)
{
	DWORD dwDataSize = 0;
	ULONG64 dwCurrentOffset = 0;
	ULONG64 dwLinkOffset = 0;
...
Author         : Anand Srivastava
--------------------------------------------------------------------------------------*/
bool CS2OS2O::Save(LPCTSTR szFileName, bool bEncryptContents)
{
	DWORD dwTotalBytesOffset = 0;
	DWORD dwTotalBytesWritten = 0;
	DWORD dwDataSize = 0;
	...
	if(INVALID_SET_FILE_POINTER == SetFilePointer(hFile, sizeof(HEADER_S2OS2O) + sizeof(HEADER_S2OS2O_DATA), 0, FILE_BEGIN))
	{
		CloseHandle(hFile);
		DeleteFile(szFullFileName);
		return (false);
	...
	while(m_pTemp)
	{
		...
		dwTotalBytesWritten += dwBytesWritten;
		dwDataSize =(DWORD)(_tcslen((TCHAR *)m_pTemp->dwKey) + 1)* sizeof(TCHAR);
		dwLinkOffset = dwNodeOffset + sizeof(NODE) + dwDataSize + 4;
		if(!WriteFile(hFile, &dwLinkOffset, SIZE_OF_ONE_NODE_ELEMENT, &dwBytesWritten, 0))
		{
			m_bSaveError = true;
		...
		dwTotalBytesWritten += dwBytesWritten;
		if(!WriteFile(hFile,(TCHAR *)m_pTemp->dwKey, dwDataSize, &dwBytesWritten, 0))
		{
			m_bSaveError = true;
			break;
		}
		...
		dwTotalBytesWritten += DumpS2O(hFile, m_pTemp->dwData);
		if(m_bSaveError)
		{
			break;
		}
	...
	if(INVALID_SET_FILE_POINTER == SetFilePointer(hFile, dwTotalBytesOffset, 0, FILE_BEGIN))
	{
		CloseHandle(hFile);
		DeleteFile(szFullFileName);
		return (false);
	...
	if(!WriteFile(hFile, &dwTotalBytesWritten, 4, &dwBytesWritten, 0))
	{
		CloseHandle(hFile);
		DeleteFile(szFullFileName);
		return (false);
	...
	if(!CryptFileData(hFile, sizeof(HEADER_S2OS2O) + sizeof(HEADER_S2OS2O_DATA)))
	{
		CloseHandle(hFile);
		DeleteFile(szFullFileName);
		return (false);
	...
	if(!WriteFile(hFile, HEADER_S2OS2O, sizeof(HEADER_S2OS2O), &dwBytesWritten, 0))
	{
		CloseHandle(hFile);
		DeleteFile(szFullFileName);
		return (false);
	...
	if(bEncryptContents && !CreateHeaderData(hFile, szFullFileName, HEADER_S2OS2O_DATA,
												sizeof(HEADER_S2OS2O_DATA)))
	{
		CloseHandle(hFile);
		DeleteFile(szFullFileName);
		return (false);
	...
	if(!WriteFile(hFile, HEADER_S2OS2O_DATA, sizeof(HEADER_S2OS2O_DATA), &dwBytesWritten, 0))
	{
		CloseHandle(hFile);
		DeleteFile(szFullFileName);
		return (false);
