...
NOTES		     : Wrapper to handle SD47 Full File Databases (Small, Delete)
VERSION HISTORY  : 
======================================================================================*/
#pragma once
#include "FSDB.h"
...
typedef struct _SMALL_FULLFILE_SIG_DB_ARRAY
{
	CFSDB		objMaxPESig;
	DWORD		dwVersion;
}FULLFILESIGDBARRAY,*pFULLFILESIGDBARRAY;

class CFullFileSigDBManager
{
	//CFSDB m_arrStaticBlackDB[16];
	//CFSDB m_arrNewBlackDB[16];
	FULLFILESIGDBARRAY		**m_pDbArray;
	FULLFILESIGDBARRAY		**m_pDeleteDBArray;
	DWORD					m_dwSmallDBCnt;
	DWORD					m_dwDeleteDBCnt;
	CFSDB					m_FullFilePESig;
	...
	DWORD	GetSD47Ver(LPCTSTR pszFileName);
	void	RemoveSmallDBs();

public:
	CFullFileSigDBManager();
	~CFullFileSigDBManager();
