...
CFullFileSigDBManager::CFullFileSigDBManager()
{
	m_dwSmallDBCnt = 0x00;
	m_dwDeleteDBCnt = 0x00;
	m_pDbArray = nullptr;
	m_pDeleteDBArray = nullptr;
...
DWORD	CFullFileSigDBManager::GetSD47Ver(LPCTSTR pszFileName)
{
	DWORD		dwRetVal = 0x00;
	CString		csFileName;
	...
	csFileName.Replace(L"SD47_",L"");
	csFileName.Replace(L"SD47D_",L"");
...
DWORD	CFullFileSigDBManager::GetHeighestVersion()
{
	DWORD	dwRetValue = 470000000;
...
DWORD	CFullFileSigDBManager::LoadSmallDBs(LPCTSTR pszDBFolderName, bool bCheckVersion, bool bEncryptData, bool * pbDeleteIfFail)
{
	...
	_stprintf(szDBFolPath,L"%sSD47_47*.db",pszDBFolderName);
	...
	while(bFind)
	{
		...
		if (objDbFinder.IsDirectory() == FALSE && objDbFinder.IsDots() == FALSE)
		{
			...
			m_pDbArray[dwDBIndex]->dwVersion = GetSD47Ver(objDbFinder.GetFileName());
...
DWORD	CFullFileSigDBManager::LoadDeleteDBs(LPCTSTR pszDBFolderName, bool bCheckVersion, bool bEncryptData, bool * pbDeleteIfFail)
{
	DWORD		dwDBIndex = 0x00;
	TCHAR		szDBFolPath[MAX_PATH] = {0x00};
	TCHAR		szDBPath[MAX_PATH] = {0x00};
	...
	_stprintf(szDBFolPath,L"%sSD47D_47*.db",pszDBFolderName);
	...
	while(bFind)
	{
		...
		if (objDbFinder.IsDirectory() == FALSE && objDbFinder.IsDots() == FALSE)
		{
			...
			m_pDeleteDBArray[dwDBIndex]->dwVersion = GetSD47Ver(objDbFinder.GetFileName());
...
DWORD	CFullFileSigDBManager::GetDeleteDBCount(LPCTSTR pszDBFolderName)
{
...
DWORD	CFullFileSigDBManager::GetSmallDBCount(LPCTSTR pszDBFolderName)
{
	DWORD		dwDBFileCnt = 0x00;
	TCHAR		szDBFolPath[MAX_PATH] = {0x00};
	BOOL		bFind = FALSE;
...
bool CFullFileSigDBManager::CreateDeleteDBArray()
{
	...
	m_pDeleteDBArray = (FULLFILESIGDBARRAY **)calloc(m_dwDeleteDBCnt,sizeof(FULLFILESIGDBARRAY *));
	...
	{
		m_pDeleteDBArray[i] = new FULLFILESIGDBARRAY;
...
bool CFullFileSigDBManager::CreateSmallDBArray()
{
	bool	bArrayCreated = false;
	...
	//Reminder for Ravi : Need to Add Error Handling
	m_pDbArray = (FULLFILESIGDBARRAY **)calloc(m_dwSmallDBCnt,sizeof(FULLFILESIGDBARRAY *));
	...
	{
		m_pDbArray[i] = new FULLFILESIGDBARRAY;
...
bool CFullFileSigDBManager::Load(LPCTSTR szFolderName, bool bCheckVersion, bool bEncryptData, bool * pbDeleteIfFail)
{
	...
	_stprintf(szLargeDbFileName,L"%s%s",szFolderName , SD_DB_FS_FULLFILE_MD5);
	bStatus = m_FullFilePESig.Load(szLargeDbFileName,bCheckVersion,bEncryptData,pbDeleteIfFail);
	if (bStatus == false)
	{
		return bStatus;
	}
...
bool CFullFileSigDBManager::SearchSigInDeleteDbs(PULONG64 pSig2Search, LPDWORD pSpyID, DWORD dwSigMatchVer)
{
	bool	bResult = false;
...
bool CFullFileSigDBManager::SearchSigInSmallDs(PULONG64 pSig2Search, LPDWORD pSpyID, DWORD dwLocalVer, bool *pbFoundInDelete)
{
	bool	bResult = false;
...
bool CFullFileSigDBManager::SearchSig(PULONG64 pSig, LPDWORD pSpyID, DWORD dwLocalVer)
{
	DWORD			dwIndex = 0x00;
	bool			bResult = false;
	bool			bDeleteResult = false;
	...
	if (bResult == false && bFoundInDelete == false)
	{
		...
		{
			bResult = m_FullFilePESig.SearchSig(pSig, pSpyID);
			if (bResult)
			{
...
bool CFullFileSigDBManager::RemoveAll(bool bRemoveTree)
{
	m_FullFilePESig.RemoveAll(bRemoveTree);
	RemoveSmallDBs();
	return true;
}
void CFullFileSigDBManager::RemoveSmallDBs()
{
...
*/
/*
//Tushar ==> Commeneted this as we do not need these Functionalies any more
...
bool CBlackDBManager::MergeObject(CMaxNewPESig& objToAdd, bool bAdd)
{
	...
	while(1)
	{
		...
		for(int iIndex = 0x00; iIndex < dwRet; iIndex++)
		{
			dwSpyID = 0x00;
