...
/*-----------------------------------------------------------------------------
Function		: CXComboList(CONSTRUCTOR)
In Parameters	: CWnd* :  CWnd class pointer
Out Parameters	:
Purpose		:initialize CXComboList class
Author		:
-----------------------------------------------------------------------------*/
CXComboList::CXComboList(CWnd *pParent)
{
	...
	{
		AddLogEntry(_T("Exception caught in CXComboList::CXComboList"));
	}
}

/*-----------------------------------------------------------------------------
Function		: ~CXComboList(DESTRUCTOR)
In Parameters	:
Out Parameters	:
Purpose		:destruct CXComboList class
Author		:
-----------------------------------------------------------------------------*/
...
/*-----------------------------------------------------------------------------
Function		: SetActive
In Parameters	: int  :  contains width of scroll Bar
Out Parameters	:
Purpose		:adjust the scroll bar and redraw window
Author		:
-----------------------------------------------------------------------------*/
void CXComboList::SetActive(int nScrollBarWidth)
{
	...
	{
		AddLogEntry(_T("Exception caught in CXComboList::SetActive"));
	}
}

/*-----------------------------------------------------------------------------
Function		: GetScrollBarCtrl
In Parameters	: int :
Out Parameters	: CScrollBar* : pointer to CScrollBar class
Purpose		:returns Scroll bar controll
Author		:
-----------------------------------------------------------------------------*/
CScrollBar* CXComboList::GetScrollBarCtrl(int nBar)
{
	...
	{
		AddLogEntry(_T("Exception caught in CXComboList::GetScrollBarCtrl"));
	}
}

/*-----------------------------------------------------------------------------
Function		: SendRegisteredMessage
In Parameters	: UINT :  Specifies the message to be sent.
: WPARAM : additional message-dependent information.
: LPARAM : additional message-dependent information.
Out Parameters	:
Purpose		:This Function calls the window procedure
Author		:
-----------------------------------------------------------------------------*/
void CXComboList::SendRegisteredMessage(UINT nMsg, WPARAM wParam, LPARAM lParam)
{
	...
	{
		AddLogEntry(_T("Exception caught in CXComboList::SendRegisteredMessage"));
	}
}
...
void CXComboList::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	{
		AddLogEntry(_T("Exception caught in CXComboList::OnLButtonDown"));
	}
}

/*-----------------------------------------------------------------------------
Function		: PreTranslateMessage
In Parameters	:MSG : Specifies the message
Out Parameters	:
Purpose		: This function handles the message
Author		:
-----------------------------------------------------------------------------*/
BOOL CXComboList::PreTranslateMessage(MSG* pMsg)
{
	...
	{
		AddLogEntry(_T("Exception caught in CXComboList::PreTranslateMessage"));
	}
	return FALSE;
}
...
In Parameters	: CWnd* :  CWnd class pointer
Out Parameters	:
Purpose		:The framework calls this member function immediately before
...
void CXComboList::OnKillFocus(CWnd* pNewWnd)
{
	...
	{
		AddLogEntry(_T("Exception caught in CXComboList::OnKillFocus"));
	}
}
...
int CXComboList::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	{
		AddLogEntry(_T("Exception caught in CXComboList::OnCreate"));
	}
	return -1;
}
...
/*-----------------------------------------------------------------------------
Function		: OnVScroll
In Parameters	: UINT :  Specifies a scroll-bar code that indicates
the user's scrolling request
: UINT : Contains the current scroll-box position if the scroll-bar
code is SB_THUMBPOSITION or SB_THUMBTRACK; otherwise not used
: CScrollBar : contains a pointer to the control.
Out Parameters	:
Purpose		:The framework calls this member function when the user clicks
...
void CXComboList::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar*)
{
	...
	{
		AddLogEntry(_T("Exception caught in CXComboList::OnVScroll"));
	}
}
...
CWnd object that it is being destroyed.
Author		:
-----------------------------------------------------------------------------*/
void CXComboList::OnDestroy()
{
	...
	{
		AddLogEntry(_T("Exception caught in CXComboList::OnDestroy"));
	}
}
...
void CXComboList::OnTimer(UINT_PTR nIDEvent)
{
	...
	{
		AddLogEntry(_T("Exception caught in CXComboList::OnTimer"));
	}
}
