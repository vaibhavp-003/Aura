...
BEGIN_MESSAGE_MAP(CTrayDialog, CDialog)
	...
	ON_WM_DESTROY()
	ON_WM_SYSCOMMAND()
	ON_MESSAGE(WM_TRAY_ICON_NOTIFY_MESSAGE,&CTrayDialog::OnTrayNotify)
	//}}AFX_MSG_MAP
	ON_WM_CTLCOLOR()
END_MESSAGE_MAP()
...
{
	HICON hIcon = 0;
	hIcon = LoadIcon(theApp.m_hResDLL, lpszResourceName);
	if(hIcon)
	{
		m_nidIconData.hIcon = hIcon;
		m_nidIconData.uFlags |= NIF_ICON;
...
void CTrayDialog::TraySetToolTip(LPCTSTR lpszToolTip)
{
	...
	wcscpy_s(m_nidIconData.szTip, _countof(m_nidIconData.szTip), lpszToolTip);
...
In Parameters	: -
Out Parameters	: BOOL
Purpose			: To set values to the controls
Author			: Ramkrushna Shelke
--------------------------------------------------------------------------------------*/
BOOL CTrayDialog::OnInitDialog()
{
	CDialog::OnInitDialog();

	m_objUIBGBrush.CreateSolidBrush(RGB(170, 1, 4));
...
Author			: Ramkrushna Shelke
--------------------------------------------------------------------------------------*/
HBRUSH CTrayDialog::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
	HBRUSH hbr = CDialog::OnCtlColor(pDC, pWnd, nCtlColor);

	hbr = (HBRUSH)m_objUIBGBrush.m_hObject;
