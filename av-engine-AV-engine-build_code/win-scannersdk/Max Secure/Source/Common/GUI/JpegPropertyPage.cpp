...
#include "JpegPropertyPage.h"

CJpegPropertyPage::CJpegPropertyPage(UINT nTemplateID)
	: CPropertyPage(nTemplateID)
{
	m_pPicture		   = NULL;
...
CJpegPropertyPage::~CJpegPropertyPage()
{
	if (m_pPicture)
	{
		m_pPicture->Release();
...
BEGIN_MESSAGE_MAP(CJpegPropertyPage, CPropertyPage)
	ON_WM_ERASEBKGND()
END_MESSAGE_MAP()

BOOL CJpegPropertyPage::OnEraseBkgnd(CDC *pDC)
{
...
BOOL CJpegPropertyPage::Load(LPCTSTR szResourceName, LPCTSTR szResourceType)
{
...
BOOL CJpegPropertyPage::Load(HGLOBAL hGlobal, DWORD dwSize)
{
...
BOOL CJpegPropertyPage::PrepareDC(int nWidth, int nHeight)
{
