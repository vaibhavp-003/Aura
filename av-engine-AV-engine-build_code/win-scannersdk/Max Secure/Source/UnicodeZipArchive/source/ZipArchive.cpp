...
CString CZipArchive::GetFileDirAndName(LPCTSTR lpszFilePath)
{
	...
	CString csExt(szExt);
	if((csExt.CompareNoCase(_T(".sd")) == 0) && csFileName.Find('.'))
		csExt = L"";
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
bool CZipArchive::Extract(CString csZipFile,CString csExtractPath )
{
	BOOL berr;
	try
	{
...
#ifdef _UNICODE
		if(_waccess_s(szFile, 0) != 0)
#else
		if(_access_s(szFile, 0) != 0)
#endif
		{
...
int CZipArchive::WideToSingle(LPCTSTR lpWide, CZipAutoBuffer &szSingle)
{
	...
	wideLen = (wideLen*sizeof(TCHAR)) + sizeof(TCHAR);
...
#else // if not UNICODE just copy
	// 	iLen does not include the NULL character
	//singleLen = (singleLen*sizeof(TCHAR)) + sizeof(TCHAR);
