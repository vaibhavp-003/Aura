...
int ExtractArchiveFile(TCHAR * chAction, TCHAR * chArchiveName, TCHAR * chInOutputFolderName,TCHAR *chPassword,TCHAR *chFileFilter)
{
 ...
 commandStrings.Add((UString)"-y");
  ...
  if (options.EnableHeaders)
  {
	...
	parser.Parse2(options/*,(FString)chOutputFolderName*/);
...
--------------------------------------------------------------------------------------*/
BOOL WINAPI __stdcall UnMax7zArchive(TCHAR * chAction, TCHAR * chArchiveName, TCHAR * chOutputFolderName, TCHAR * chPassword,TCHAR * chFileFilter)
{
	int iRet = ExtractArchiveFile(chAction,chArchiveName,chOutputFolderName,chPassword,chFileFilter);
	if(iRet == 0)
		return true;
	else
		return false;			
...
Description		: It is a exported function which calls "ExtractFile" for extraction purpose
--------------------------------------------------------------------------------------*/
BOOL WINAPI __stdcall Max7zArchive(TCHAR * chAction, TCHAR * chArchiveName, TCHAR * chInputFileFolderName, TCHAR * chPassword,TCHAR * chFileFilter)
{
	int iRet = ExtractArchiveFile(chAction,chArchiveName,chInputFileFolderName,chPassword,chFileFilter);
	if(iRet == 0)
		return true;
	else
		return false;			
