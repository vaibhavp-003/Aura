/*=============================================================================
   FILE		           : ZipFileHeaderA.cpp
   ABSTRACT		       : implementation of the CZipFileHeaderA class.
   DOCUMENTS	       : 
   AUTHOR		       : 
   COMPANY		       : Aura 
   COPYRIGHT NOTICE    :
...
#define ZipFileHeaderASIZE	46
#define LOCALZipFileHeaderASIZE	30
#define VERSIONMADEBY 20
#define ENCR_HEADER_LEN 12

char CZipFileHeaderA::m_gszSignature[] = {0x50, 0x4b, 0x01, 0x02};
char CZipFileHeaderA::m_gszLocalSignature[] = {0x50, 0x4b, 0x03, 0x04};
/*-------------------------------------------------------------------------------------
	Function       : ~CZipFileHeaderA
	Purpose		   : Constructor for class CZipFileHeaderA
	Author		   : 
-------------------------------------------------------------------------------------*/
CZipFileHeaderA::CZipFileHeaderA()
{
...
/*-------------------------------------------------------------------------------------
	Function       : ~CZipFileHeaderA
	Purpose		   : Destructor for class CZipFileHeaderA
	Author		   : 
-------------------------------------------------------------------------------------*/
CZipFileHeaderA::~CZipFileHeaderA()
{
...
/*-------------------------------------------------------------------------------------
	Function       : Read
	In Parameters  : CZipStorageA *pStorage
	Out Parameters : bool
	Purpose		   : Read the header from the central dir
	Author		   : 
-------------------------------------------------------------------------------------*/
bool CZipFileHeaderA::Read(CZipStorageA *pStorage)
{
	WORD uFileNameSize, uCommentSize, uExtraFieldSize;
	CZipAutoBufferA buf(ZipFileHeaderASIZE);
	pStorage->Read(buf, ZipFileHeaderASIZE, true);		
...
/*-------------------------------------------------------------------------------------
	Function       : ReadLocal
	In Parameters  : CZipStorageA *pStorage, WORD& iLocExtrFieldSize
	Out Parameters : bool
	Purpose		   :  read local header
	Author		   : 
-------------------------------------------------------------------------------------*/
bool CZipFileHeaderA::ReadLocal(CZipStorageA *pStorage, WORD& iLocExtrFieldSize)
{
	char buf[LOCALZipFileHeaderASIZE];
	pStorage->Read(buf, LOCALZipFileHeaderASIZE, true);
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
void CZipFileHeaderA::SetTime(const CTime &time)
{
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
bool CZipFileHeaderA::CheckCrcAndSizes(char *pBuf)
{
...
/*-------------------------------------------------------------------------------------
	Function       : WriteLocal
	In Parameters  : CZipStorageA& storage
	Out Parameters : void
	Purpose		   : write the local header
	Author		   : 
-------------------------------------------------------------------------------------*/
void CZipFileHeaderA::WriteLocal(CZipStorageA& storage)
{
	// extra field is local by now
	WORD uFileNameSize = GetFileNameSize(),	uExtraFieldSize = GetExtraFieldSize();
	DWORD iLocalSize = LOCALZipFileHeaderASIZE + uExtraFieldSize + uFileNameSize;
	CZipAutoBufferA buf(iLocalSize);
	...
	// it was only local information, use CZipArchiveA::SetExtraField to set the file extra field in the central directory
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
bool CZipFileHeaderA::PrepareData(int iLevel, bool bExtraHeader, bool bEncrypted)
{
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
void CZipFileHeaderA::GetCrcAndSizes(char * pBuffer)
{
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
DWORD CZipFileHeaderA::GetSize()
{
	return ZipFileHeaderASIZE + GetExtraFieldSize() + GetFileNameSize() + GetCommentSize();
}
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
bool CZipFileHeaderA::IsEncrypted()
{
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
bool CZipFileHeaderA::IsDataDescr()
{
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
bool CZipFileHeaderA::SetComment(LPCTSTR lpszComment)
{
	return CZipArchiveA::WideToSingle(lpszComment, m_pszComment)	!= -1;
}
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
CString CZipFileHeaderA::GetComment()
{
	CString temp;
	CZipArchiveA::SingleToWide(m_pszComment, temp);
	return temp;
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
bool CZipFileHeaderA::SetFileName(LPCTSTR lpszFileName)
{
	return CZipArchiveA::WideToSingle(lpszFileName, m_pszFileName) != -1;
}
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
CString CZipFileHeaderA::GetFileName()
{
	CString temp;
	CZipArchiveA::SingleToWide(m_pszFileName, temp);
	return temp;
}
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
void CZipFileHeaderA::SlashChange(bool bWindowsStyle)
{
	char t1 = '\\', t2 = '/', c1, c2;
	if (bWindowsStyle)
	{
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
void CZipFileHeaderA::AnsiOem(bool bAnsiToOem)
{
	if (bAnsiToOem)
