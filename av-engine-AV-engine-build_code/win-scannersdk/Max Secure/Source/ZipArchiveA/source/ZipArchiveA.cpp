/*=============================================================================
   FILE		           : ZipArchiveA.cpp
   ABSTRACT		       : implementation of the CZipArchiveA class.
   DOCUMENTS	       : 
   AUTHOR		       : 
   COMPANY		       : Aura 
   COPYRIGHT NOTICE    :
...
/*-------------------------------------------------------------------------------------
	Function       : CZipArchiveA
	Purpose		   : Constructor for class CZipArchiveA
	Author		   : 
-------------------------------------------------------------------------------------*/
TCHAR CZipArchiveA::m_gszCopyright[] = {_T("Zip archive creation and modification Copyright 2000 Tadeusz Dracz")};
CZipArchiveA::CZipArchiveA()
{
...
/*-------------------------------------------------------------------------------------
	Function       : ~CZipArchiveA
	Purpose		   : Destructor for class CZipArchiveA
	Author		   : 
-------------------------------------------------------------------------------------*/
CZipArchiveA::~CZipArchiveA()
{
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
void CZipArchiveA::Open(LPCTSTR szPathName, int iMode, int iVolumeSize)
{
	if (!IsClosed())
	{
		TRACE(_T("ZipArchiveA already opened.\n"));
		return;
	}
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
void CZipArchiveA::Open(CMemFile& mf, int iMode)
{
	if (!IsClosed())
	{
		TRACE(_T("ZipArchiveA already opened.\n"));
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
bool CZipArchiveA::IsClosed(bool bArchive)
{
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
void CZipArchiveA::ThrowError(int err)
{
	AfxThrowZipExceptionA(err, IsClosed() ? _T("") : m_storage.m_pFile->GetFilePath());
}
/*-------------------------------------------------------------------------------------
	Function       : DeleteFile
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
bool CZipArchiveA::DeleteFile(WORD uIndex)
{
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
int CZipArchiveA::GetNoEntries()
{
	return (int)m_centralDir.m_headers.GetSize();
}
/*-------------------------------------------------------------------------------------
	Function       : GetFileInfo
	In Parameters  : CZipFileHeaderA & fhInfo, WORD uIndex
	Out Parameters : bool
	Purpose		   : Retrive the All file info and store in CZipFileHeaderA class
	Author		   : 
-------------------------------------------------------------------------------------*/
bool CZipArchiveA::GetFileInfo(CZipFileHeaderA & fhInfo, WORD uIndex)
{
	if (IsClosed())
	{
		TRACE(_T("ZipArchiveA is closed.\n"));
		return false;
	}
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
int CZipArchiveA::FindFile(CString szFileName, bool bCaseSensitive)
{
	if (IsClosed())
	{
		TRACE(_T("ZipArchiveA is closed.\n"));
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
bool CZipArchiveA::OpenFile(WORD uIndex)
{
	if (!m_centralDir.IsValidIndex(uIndex))
		return false;
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
int CZipArchiveA::GetLocalExtraField(char *pBuf, int iSize)
{
	if (IsClosed())
	{
		TRACE(_T("ZipArchiveA is closed.\n"));
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
void* CZipArchiveA::myalloc(void* opaque, UINT items, UINT size)
{
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
void CZipArchiveA::myfree(void* opaque, void* address)
{
	if (opaque)
	{
		CPtrList* list  = (CPtrList*) opaque;
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
void CZipArchiveA::CheckForError(int iErr)
{
...
/*-------------------------------------------------------------------------------------
	...
	In Parameters  : void
	Out Parameters : CZipFileHeaderA*
	Purpose		   : Return the current file header info
	Author		   : 
-------------------------------------------------------------------------------------*/
CZipFileHeaderA* CZipArchiveA::CurrentFile()
{
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
DWORD CZipArchiveA::ReadFile(void *pBuf, DWORD iSize)
{
	if (m_iFileOpened != extract)
	{
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
void CZipArchiveA::SetSpanCallback(ZIPCALLBACKFUN pFunc, void* pData)
{
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
void CZipArchiveA::SetAdvanced(int iWriteBuffer, int iExtractBuffer, int iSearchBuffer)
{
	if (!IsClosed())
	{
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
int CZipArchiveA::CloseFile(CFile &file)
{
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
int CZipArchiveA::CloseFile(LPCTSTR lpszFilePath, bool bAfterException)
{
	if (m_iFileOpened != extract)
	{
...
/*-------------------------------------------------------------------------------------
	...
	In Parameters  : CZipFileHeaderA & header, int iLevel, LPCTSTR lpszFilePath
	Out Parameters : bool
	Purpose		   : add a new file to the zip archive
	Author		   : 
-------------------------------------------------------------------------------------*/
bool CZipArchiveA::OpenNewFile(CZipFileHeaderA & header, int iLevel, LPCTSTR lpszFilePath)
{
	if (IsClosed())
	{
		TRACE(_T("ZipArchiveA is closed.\n"));
	...
	if (bEncrypted)
	{
		CZipAutoBufferA buf(ENCR_HEADER_LEN);
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
CString CZipArchiveA::GetFilePath(LPCTSTR lpszFilePath)
{
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
CString CZipArchiveA::GetFileExt(LPCTSTR lpszFilePath)
{
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
CString CZipArchiveA::GetFileTitle(LPCTSTR lpszFilePath)
{
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
CString CZipArchiveA::GetFileDirAndName(LPCTSTR lpszFilePath)
{
	TCHAR szDir[_MAX_DIR];
	TCHAR szFname[_MAX_FNAME];
	TCHAR szExt[_MAX_EXT];
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
CString CZipArchiveA::GetFileName(LPCTSTR lpszFilePath)
{
	TCHAR szExt[_MAX_EXT];
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
bool CZipArchiveA::ForceDirectory(LPCTSTR lpDirectory)
{
	ASSERT(lpDirectory);
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
int CZipArchiveA::FileExists(LPCTSTR lpszName)
{
	CFileFind fileFind;
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
bool CZipArchiveA::DirectoryExists(LPCTSTR lpszDir)
{
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
bool CZipArchiveA::IsFileDirectory(WORD uIndex)
{
	if (IsClosed())
	{
		TRACE(_T("ZipArchiveA is closed.\n"));
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
bool CZipArchiveA::Extract(CString csZipFile,CString csExtractPath , LPCTSTR szPassword)
{
	BOOL berr;
	try
	{
		berr = 0;
		Open(csZipFile, CZipArchiveA::open, 0 );
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
bool CZipArchiveA::ExtractFile(WORD uIndex,
							  LPCTSTR lpszPath,             
                              bool bFullPath,              
                              LPCTSTR lpszNewName,          
                              ZIPCALLBACKFUN pCallback,     
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
void CZipArchiveA::SetExtraField(char *pBuf, WORD iSize)
{
	if (m_iFileOpened != compress)
	{
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
bool CZipArchiveA::WriteNewFile(void *pBuf, DWORD iSize)
{
	if (m_iFileOpened != compress)
	{
		TRACE(_T("A new file must be opened.\n"));
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
bool CZipArchiveA::CloseNewFile()
{
	if (m_iFileOpened != compress)
	{
		TRACE(_T("A new file must be opened.\n"));
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
void CZipArchiveA::DeleteFiles(CStringArray &aNames, bool bCaseSensitive)
{
	CWordArray indexes;
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
void CZipArchiveA::DeleteFiles(CWordArray &aIndexes)
{
	if (IsClosed())
	{
		TRACE(_T("ZipArchiveA is closed.\n"));
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
DWORD CZipArchiveA::RemovePackedFile(DWORD uStartOffset, DWORD uEndOffset)
{
	...
	if (uTotalToWrite != TotalWritten)
		ThrowError(CZipExceptionA::generic);
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
void CZipArchiveA::DeleteInternal(WORD uIndex)
{
	CZipFileHeaderA* pfh = m_centralDir.m_headers[uIndex];
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
bool CZipArchiveA::IsDriveRemovable(LPCTSTR lpszFilePath)
{
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
CString CZipArchiveA::GetDrive(LPCTSTR lpszFilePath)
{
	TCHAR szDrive[_MAX_DRIVE];
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
bool CZipArchiveA::AddNewFile(LPCTSTR lpszFilePath,  
							 int iLevel,          
                             bool bFullPath,      
                             ZIPCALLBACKFUN pCallback,
                             void* pUserData,         
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
CString CZipArchiveA::GetArchivePath()
{
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
CString CZipArchiveA::GetGlobalComment()
{
	if (IsClosed())
	{
		TRACE(_T("ZipArchiveA is closed.\n"));
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
bool CZipArchiveA::SetGlobalComment(const CString &szComment)
{
	if (IsClosed())
	{
		TRACE(_T("ZipArchiveA is closed.\n"));
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
bool CZipArchiveA::IsDirectory(DWORD uAttr)
{
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
int CZipArchiveA::GetCurrentDisk()
{
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
bool CZipArchiveA::SetFileComment(WORD uIndex, CString szComment)
{
	if (IsClosed())
	{
		TRACE(_T("ZipArchiveA is closed.\n"));
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
int CZipArchiveA::CompareWords(const void *pArg1, const void *pArg2)
{
	WORD w1 = *(WORD*)pArg1;
	WORD w2 = *(WORD*)pArg2;
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
void CZipArchiveA::CryptInitKeys()
{
...
-------------------------------------------------------------------------------------*/
void CZipArchiveA::CryptUpdateKeys(char c)
{	
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
char CZipArchiveA::CryptDecryptByte()
{
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
void CZipArchiveA::CryptDecode(char &c)
{
	c ^= CryptDecryptByte();
	CryptUpdateKeys(c);
}
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
bool CZipArchiveA::SetPassword(LPCTSTR lpszPassword)
{
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
DWORD CZipArchiveA::CryptCRC32(DWORD l, char c)
{
...
/*-------------------------------------------------------------------------------------
	...
	In Parameters  : long iCrc, CZipAutoBufferA &buf
	Out Parameters : void
	Purpose		   : Crypt the header
	Author		   : 
-------------------------------------------------------------------------------------*/
void CZipArchiveA::CryptCryptHeader(long iCrc, CZipAutoBufferA &buf)
{
	CryptInitKeys();
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
void CZipArchiveA::CryptEncode(char &c)
{
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
void CZipArchiveA::CryptEncodeBuffer()
{
	if (CurrentFile()->IsEncrypted())
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
void CZipArchiveA::CloseFileAfterTestFailed()
{
	if (m_iFileOpened != extract)
	{
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
bool CZipArchiveA::TestFile(WORD uIndex, ZIPCALLBACKFUN pCallback, void* pUserData, DWORD uBufSize)
{
	if (!uBufSize)
		return false;
	CZipFileHeaderA* pHeader = m_centralDir.m_headers[uIndex];
	if (IsFileDirectory(uIndex))
	{
		...
		if ((iSize != 0 || iSize != pHeader->m_uUncomprSize)
			...
			ThrowError(CZipExceptionA::dirWithSize);
		return true;
	}
	else
	{
...
/*-------------------------------------------------------------------------------------
	...
	In Parameters  : LPCTSTR lpWide, CZipAutoBufferA &szSingle
	Out Parameters : int
	Purpose		   : convert wide character to Multibyte
	Author		   : 
-------------------------------------------------------------------------------------*/
int CZipArchiveA::WideToSingle(LPCTSTR lpWide, CZipAutoBufferA &szSingle)
{
...
/*-------------------------------------------------------------------------------------
	...
	In Parameters  : CZipAutoBufferA &szSingle, CString& szWide
	Out Parameters : int
	Purpose		   : Covert the MultiByte to Widecharacter
	Author		   : 
-------------------------------------------------------------------------------------*/
int CZipArchiveA::SingleToWide(CZipAutoBufferA &szSingle, CString& szWide)
{
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
const DWORD* CZipArchiveA::GetCRCTable()
{
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
void CZipArchiveA::CryptDecodeBuffer(DWORD uCount)
{
	if (CurrentFile()->IsEncrypted())
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
void CZipArchiveA::EmptyPtrList()
{
	if (m_list.GetCount())
	{
...
/*-------------------------------------------------------------------------------------
	...
	Purpose		   : Enable fast finding by the file name of the files inside the archive.
					Set CZipCentralDirA::m_bFindFastEnabled to true, which is required by FindFile.
					Do not enable it, if you don't plan to use FindFile function

	Author		   : 
-------------------------------------------------------------------------------------*/
void CZipArchiveA::EnableFindFast(bool bEnable)
{
	if (IsClosed())
	{
