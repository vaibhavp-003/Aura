/*=============================================================================
   FILE		           : ZipCentralDirA.cpp
   ABSTRACT		       : implementation of the CZipCentralDirA class.
   DOCUMENTS	       : 
   AUTHOR		       : 
   COMPANY		       : Aura 
   COPYRIGHT NOTICE    :
...
struct CZipAutoHandle
{
	...
	bool CreateMapping(HANDLE hFile)
	{
		m_hFileMap = CreateFileMapping(hFile, NULL, PAGE_READWRITE,
			0, 0, _T("ZipArchiveA Mapping File"));
		if (!m_hFileMap)
			return false;
...
char CZipCentralDirA::m_gszSignature[] = {0x50, 0x4b, 0x05, 0x06};
/*-------------------------------------------------------------------------------------
	Function       : CZipCentralDirA
	Purpose		   : Constructor for class ZipCentralDirA
	Author		   : 
-------------------------------------------------------------------------------------*/
CZipCentralDirA::CZipCentralDirA()
{
...
/*-------------------------------------------------------------------------------------
	Function       : !CZipCentralDirA
	Purpose		   : Constructor for class CZipCentralDirA
	Author		   : 
-------------------------------------------------------------------------------------*/
CZipCentralDirA::~CZipCentralDirA()
{
...
/*-------------------------------------------------------------------------------------
	...
	Purpose		   : return the location of the beginning of the "end" record in the file
	Author		   : 
-------------------------------------------------------------------------------------*/

DWORD CZipCentralDirA::Locate()
{
	...
	CZipAutoBufferA buf(m_iBufferSize);
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
void CZipCentralDirA::ThrowError(int err)
{
	AfxThrowZipExceptionA(err, m_pStorage->m_pFile->GetFilePath());
}
...
/*-------------------------------------------------------------------------------------
	...
	In Parameters  : 
	Out Parameters : 
	Purpose		   : Read file headers
	Author		   : 
-------------------------------------------------------------------------------------*/
void CZipCentralDirA::ReadHeaders()
{
	m_pStorage->m_pFile->Seek(m_uOffset + m_uBytesBeforeZip, CFile::begin);
	RemoveHeaders();
	for (int i = 0; i < m_uEntriesNumber; i++)
	{
		CZipFileHeaderA* pHeader = new CZipFileHeaderA;
...
/*-------------------------------------------------------------------------------------
	Function       : Clear
	In Parameters  : bool bEverything
	Out Parameters : void
	Purpose		   : remove all headers from the central dir
	Author		   : 
-------------------------------------------------------------------------------------*/
void CZipCentralDirA::Clear(bool bEverything)
{
	m_pOpenedFile = NULL;
...
/*-------------------------------------------------------------------------------------
	...
	Out Parameters : bool
	Purpose		   : Check for valid index
	Author		   : 
-------------------------------------------------------------------------------------*/
bool CZipCentralDirA::IsValidIndex(WORD uIndex)
{
...
/*-------------------------------------------------------------------------------------
	Function       : open the file for extracting
	In Parameters  : OpenFile
	Out Parameters : 
	Purpose		   : Open File by index
	Author		   : 
-------------------------------------------------------------------------------------*/
void CZipCentralDirA::OpenFile(WORD uIndex)
{
...
/*-------------------------------------------------------------------------------------
	...
	In Parameters  : 
	Out Parameters : 
	Purpose		   : Close the given open file
	Author		   : 
-------------------------------------------------------------------------------------*/
void CZipCentralDirA::CloseFile()
{
	if (!m_pOpenedFile)
		return;
	m_pLocalExtraField.Release();
	if (m_pOpenedFile->IsDataDescr())
	{
		...
		if (memcmp(buf, CZipStorageA::m_gszExtHeaderSignat, 4) != 0) // there is no signature
...
/*-------------------------------------------------------------------------------------
	Function       : AddNewFile
	In Parameters  : CZipFileHeaderA & header
	Out Parameters : void
	Purpose		   : add new header using the argument as a template
	Author		   : 
-------------------------------------------------------------------------------------*/
void CZipCentralDirA::AddNewFile(CZipFileHeaderA & header)
{
	CZipFileHeaderA* pHeader = new CZipFileHeaderA(header);
...
/*-------------------------------------------------------------------------------------
	...
	In Parameters  : 
	Out Parameters : 
	Purpose		   : called during adding or deleting files; remove the central dir from the disk
	Author		   : 
-------------------------------------------------------------------------------------*/
void CZipCentralDirA::RemoveFromDisk()
{
	if (m_bOnDisk)
	{
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
void CZipCentralDirA::CloseNewFile()
{
	CZipAutoBufferA buf(12 + 4);
	short iToWrite = 0;
...
/*-------------------------------------------------------------------------------------
	...
	In Parameters  : 
	Out Parameters : 
	Purpose		   : Write the headers of the files
	Author		   : 
-------------------------------------------------------------------------------------*/
void CZipCentralDirA::WriteHeaders()
{
	m_uDiskEntriesNo = 0;
	...
	for (int i = 0; i < m_uEntriesNumber; i++)
	{
		CZipFileHeaderA* pHeader = m_headers[i];
...
/*-------------------------------------------------------------------------------------
	Function       : WriteCentralEnd
	In Parameters  : 
	Out Parameters : DWORD
	Purpose		   : Write the central end of file
	Author		   : 
-------------------------------------------------------------------------------------*/
DWORD CZipCentralDirA::WriteCentralEnd()
{
	DWORD uSize = GetSize();
	CZipAutoBufferA buf(uSize);
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
void CZipCentralDirA::RemoveFile(WORD uIndex)
{
	CZipFileHeaderA* pHeader = m_headers[uIndex];
	if (m_bFindFastEnabled)
	{
...
/*-------------------------------------------------------------------------------------
	Function       : GetSize
	In Parameters  : bool bWhole
	Out Parameters : DWORD
	Purpose		   : Get the File size
	Author		   : 
-------------------------------------------------------------------------------------*/
DWORD CZipCentralDirA::GetSize(bool bWhole)
{
	DWORD uHeaders = 0;
	if (bWhole)
	{
...
/*-------------------------------------------------------------------------------------
	Function       : RemoveDataDescr
	In Parameters  : bool bFromBuffer
	Out Parameters : bool
	Purpose		   :  remove data descriptors from the write buffer in the disk spanning volume
					  that is one-disk only (do not remove from password encrypted files)
	Author		   : 
-------------------------------------------------------------------------------------*/
bool CZipCentralDirA::RemoveDataDescr(bool bFromBuffer)
{
	CZipAutoHandle ah;
	char* pFile = NULL;
	DWORD uSize;
...
/*-------------------------------------------------------------------------------------
	...
	In Parameters  : 
	Out Parameters : 
	Purpose		   : Rmove the headers
	Author		   : 
-------------------------------------------------------------------------------------*/
void CZipCentralDirA::RemoveHeaders()
{
		for (int i = 0; i < m_headers.GetSize(); i++)
			delete m_headers[i];
		m_headers.RemoveAll();
...
/*-------------------------------------------------------------------------------------
	...
	Author		   : 
-------------------------------------------------------------------------------------*/
void CZipCentralDirA::ConvertAll()
{
...
/*-------------------------------------------------------------------------------------
	Function       : BuildFindFastArray
	In Parameters  : 
	Out Parameters : 
	Purpose		   : 
	Author		   : 
-------------------------------------------------------------------------------------*/
void CZipCentralDirA::BuildFindFastArray()
{
...
/*-------------------------------------------------------------------------------------
	Function       : InsertFindFastElement
	In Parameters  : 
	Out Parameters : 
	Purpose		   : 
	Author		   : 
-------------------------------------------------------------------------------------*/
void CZipCentralDirA::InsertFindFastElement(CZipFileHeaderA* pHeader, WORD uIndex)
{
	CString fileName = pHeader->GetFileName();

	
	int iSize = (int)m_findarray.GetSize();
	...
	m_findarray.InsertAt(start, CZipFindFastA(pHeader, WORD(uIndex == -1 ? iSize : uIndex /* just in case */))); 
}

/*-------------------------------------------------------------------------------------
	Function       : FindFileNameIndex
	In Parameters  : LPCTSTR lpszFileName, bool bCaseSensitive
	Out Parameters : int
	Purpose		   : Find the file name index
	Author		   : 
-------------------------------------------------------------------------------------*/
int CZipCentralDirA::FindFileNameIndex(LPCTSTR lpszFileName, bool bCaseSensitive)
{
	int start = 0;
	int end = (int)m_findarray.GetUpperBound();
